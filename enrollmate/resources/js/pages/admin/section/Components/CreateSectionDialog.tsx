...
import { BookUp2, CalendarCheck, ChartBar, CornerDownRight, List, PencilLine, Plus, SquarePlus } from 'lucide-react';
...
const CreateSectionDialog = ({gradeLevels }: CreateSectionDialogProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    post(route('admin.sections.store'), {
      onSuccess: () => {
        ...
        setData(getDefaultSectionFormData());
        setOpen(false);
      },
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      setOpen(isOpen);
      if (isOpen) setData(getDefaultSectionFormData());
    }}>
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader className='p-4 gap-0'>
            <DialogTitle className='flex items-center gap-2'>
              ...
              <div className='text-2xl'>Add Section</div>
            </DialogTitle>
