...
const GradeEdit = ({enrollment, grades, settings }: GradeEditProps) => {
  ...
  const isFirstQuarterInputAllowed = getSetting('enable_first_quarter_input')?.value === 'true';
  const isSecondQuarterInputAllowed = getSetting('enable_second_quarter_input')?.value === 'true';
  const isThirdQuarterInputAllowed = getSetting('enable_third_quarter_input')?.value === 'true';
  const isFourthQuarterInputAllowed = getSetting('enable_fourth_quarter_input')?.value === 'true';
  const isFinalGradeInputAllowed = getSetting('enable_final_grade_input')?.value === 'true';
  ...
  const submitGrades = () => {
    put(route("admin.grades.update", enrollment.id), {
      onSuccess: () => {
  ...
  return (
    <div className="container">
      ...
      <div>
        <Link href={route('admin.grades.index')}><Button variant={'default'}> <ChevronLeft /> Back</Button></Link>  
      </div>
        ...
        <TableBody>
          {data.grades.map((grade, index) => ( 
            <TableRow key={grade.id} className='divide-x divide-gray-300 border-b border-gray-300 h-8'>
              ...
              <TableCell>
                <Input
                  className={`text-center ${isFirstQuarterInputAllowed ? 'border-green-600' : 'border-none'}`}
                  type="number"
                  step="0.01"
                  max={100}
                  ...
                  readOnly={!isFirstQuarterInputAllowed}
                />
              </TableCell>
              <TableCell>
                <Input
                  className={`text-center ${isSecondQuarterInputAllowed ? 'border-green-600' : 'border-none'}`}
                  type="number"
                  step="0.01"
                  max={100}
                  ...
                  readOnly={!isSecondQuarterInputAllowed}
                />
              </TableCell>
              <TableCell>
                <Input
                  className={`text-center ${isThirdQuarterInputAllowed ? 'border-green-600' : 'border-none'}`}
                  type="number"
                  step="0.01"
                  max={100}
                  ...
                  readOnly={!isThirdQuarterInputAllowed}
                />
              </TableCell>
              <TableCell>
                <Input
                  className={`text-center ${isFourthQuarterInputAllowed ? 'border-green-600' : 'border-none'}`}
                  type="number"
                  step="0.01"
                  max={100}
                  ...
                  readOnly={!isFourthQuarterInputAllowed}
                />
              </TableCell>
              <TableCell>
                <Input
                  className={`text-center ${isFinalGradeInputAllowed ? 'border-green-600' : 'border-none'}`}
                  type="number"
                  step="0.01"
                  max={100}
                  ...
                  readOnly={!isFinalGradeInputAllowed}
                />
              </TableCell>
            </TableRow>
          ))}
      ...
      <div className="flex justify-end">
        {  (
            isFirstQuarterInputAllowed ||
            isSecondQuarterInputAllowed ||
            isThirdQuarterInputAllowed ||
            isFourthQuarterInputAllowed ||
            isFinalGradeInputAllowed
          ) 
          && (
          <Button
            type="button"
            ...
            disabled={processing}
            className={`px-4 py-2 ${processing ? 'bg-gray-300 text-gray-700' : 'bg-green-700 text-white'}`}
            aria-live="assertive"
